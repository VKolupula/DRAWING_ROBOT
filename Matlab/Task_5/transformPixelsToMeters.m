...
% This function computes the scale factor for converting pixels to meters, then converts all the segments in segmentsPix to meters.

% First, create variables to represent the ranges xRange, yRange, xRangePix, and yRangePix.
...
% Calculate the two possible scale factors. The fraction factor indicates what percentage of the available space should be used.
...
% Based on the chosen scaling factor and range values, identify the new origin for the scaled drawing so that the entire drawing will be centered at the center of the drawable area.
...
% Loop through all segments and transform pixel values to meters for all coordinates. Subtract the pixel origin, multiply by the scaling factor, and add the drawing origin.
