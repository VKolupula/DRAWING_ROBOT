...
% Create a breaks variable that is a logical index with value 1 where there is a break and 0 where pixels are adjacent.

consecutiveDistance = abs(diff(coordsPix));
breaks = any(consecutiveDistance > [1 1],2);

% Use the breaks variable to construct the start and end indices of each segment. 
% Then loop through the segments and create a cell array segmentsPix, where each cell contains all the pixel coordinates for that segment.
